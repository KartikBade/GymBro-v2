...
interface ScheduleDao {
    ...
    @Query("DELETE FROM schedule WHERE name = :scheduleName")
    suspend fun deleteSchedule(scheduleName: String)

    @Query("UPDATE schedule SET name = :newName, description  = :description, daysPlannedOn = :daysPlannedOn WHERE name = :oldName")
