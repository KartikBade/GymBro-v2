...
class UserRepository(
    ...
    suspend fun getScheduleById(scheduleId: Int) =
        scheduleDatabase.getDao().getScheduleById(scheduleId)

    suspend fun getExerciseById(exerciseId: Int) =
        scheduleDatabase.getDao().getExerciseById(exerciseId)
    ...
    suspend fun deleteExercise(exerciseId: Int) =
        scheduleDatabase.getDao().deleteExercise(exerciseId)

    suspend fun editSchedule(
        scheduleId: Int,
        scheduleName: String,
        scheduleDescription: String,
        scheduleDaysPlannedOn: String
    ) = scheduleDatabase.getDao()
        .editSchedule(scheduleId, scheduleName, scheduleDescription, scheduleDaysPlannedOn)

    suspend fun editExercise(exerciseId: Int, exerciseName: String, exerciseInstructions: String) =
        scheduleDatabase.getDao().editExercise(exerciseId, exerciseName, exerciseInstructions)
    ...
    suspend fun getExercisesOfSchedule(scheduleId: Int) =
        scheduleDatabase.getDao().getExercisesOfSchedule(scheduleId)

    suspend fun insertLog(log: Log) = scheduleDatabase.getDao().insertLog(log)

    suspend fun getLogsOfExercise(exerciseId: Int) =
        scheduleDatabase.getDao().getLogsOfExercise(exerciseId)

    suspend fun findExerciseId(exerciseName: String) =
        scheduleDatabase.getDao().findExerciseId(exerciseName)

    suspend fun editLog(weight: Int, reps: Int, logId: Int) =
        scheduleDatabase.getDao().editLog(weight, reps, logId)
    ...
    suspend fun dataImport(): Boolean {
        ...
        try {
            ...
            if (scheduleList.isEmpty()) {
                throw java.lang.Exception("No Data")
            }
            scheduleDatabase.getDao().deleteAllExercises()
            scheduleDatabase.getDao().deleteAllLogs()
            scheduleDatabase.getDao().deleteAllSchedules()
            ...
            for (i in crossRefList) {
                insertScheduleExerciseCrossRef(i)
            }
        } catch (e: java.lang.Exception) {
            android.util.Log.e("UserRepoError", e.toString())
            deleteDatabase()
            return false
        }
        return true
